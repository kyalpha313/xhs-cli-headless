...
这份文档说明 `xhs-cli-headless` `0.8.9` 当前对外承诺的能力边界。这里的“支持”指默认命令面和 Agent skill 都可以路由；“隐藏 / 不承诺”指源码可能存在，但不建议 Agent 默认调用。
...
| `xhs login --qr-output <png>` | 支持 | 为飞书 / 微信等聊天渠道写出可发送的二维码 PNG |
